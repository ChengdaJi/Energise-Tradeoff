...
function optimal_stoach_scenario(current_time, obj, feedback, pd, pg, price, ancillary_type)
    ...
    for bank=1:BN
        ...
        @constraint(m,P_hat_rt[bank,1]==P_rt[bank,1]);
        @constraint(m,Q_hat_rt[bank,1]==Q_rt[bank,1]);
        @constraint(m,v_rt[bank,1]==v_0_rt[1,1]-2*(r[bank]*P_hat_rt[bank,1]+x[bank]*Q_hat_rt[bank,1]));
        #@constraint(m, [v_0_rt[1,1], P_hat_rt[bank,1], Q_hat_rt[bank,1]] in SecondOrderCone());
    ...
    for scenario = 1:SN
        for t=1:T-1
            ...
            for bank=1:BN
                ...
                @constraint(m,P_hat[scenario,bank,t]==P_[scenario,bank,t]);
                @constraint(m,Q_hat[scenario,bank,t]==Q_[scenario,bank,t]);
                @constraint(m,v[scenario,bank,t]==v_0[scenario,t]-
                    2*(r[bank]*P_hat[scenario,bank,t]+x[bank]*Q_hat[scenario,bank,t]));
                #@constraint(m, [v_0[scenario,t], P_hat[scenario,bank,t], Q_hat[scenario,bank,t]] in SecondOrderCone());
    ...
    P_0_o = calculate_P0_real(pg, pd, price, R_o, obj, TB);
    ...
    v = (v_o), v_0=(v_0_o), P_rsrv = (P_rsrv_o), B_rsrv = (B_rsrv_o), P_0 = (P_0_o),
...
function calculate_P0_real(pg, pd, price, R, obj, TB)
    # calculate
