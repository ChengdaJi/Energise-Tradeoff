...
function pg_traj(t, pg_raw, pg_noise, solar_error_max, p_rate, T, Pred_length);
    ...
    mu_ct = rt_raw[:,t+1];
    pred = zeros(12, Pred_length)
    solar_error_max_end = solar_error_max + 0.005*(Pred_length-1)
    traj025 = solar_error_max

    traj025 = 0.025:0.005:0.025+11*0.005

    traj = solar_error_max:0.005:solar_error_max_end
    ...
    mu = mu_scenario;
    ratio = reshape(solar_error_max:0.005:solar_error_max_end, 1, Pred_length)
    pred_square = pred.^2;
    sigma = hcat(ones(12,1)*ratio.*pred_square, p_rate^2*2*solar_error_max_end*da.^2);
